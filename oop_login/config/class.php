...
// Proses Login
	...
	{
		$query = $this->conn->query("SELECT * FROM users WHERE id_users='$id'");
	...
	function input_user($nama_users,$username,$pwd,$level){
		$this->conn->query("INSERT INTO users (nama_users,username,password,level) VALUES ('$nama_users','$username','$pwd','$level')");
	}

	// tampilkan data dari tabel users yang akan di edit 
	function edit_user($id){
		$data=$this->conn->query("SELECT * FROM users WHERE id_users='$id'");
	...
	function update_user($id,$nama_users,$username,$pwd,$level){
		$this->conn->query("UPDATE users SET nama_users='$nama_users',username='$username',password='$pwd',level='$level' WHERE id_users='$id'");
	}

	// proses delete data user
	function hapus_user($id){
		$this->conn->query("DELETE FROM users where id_users='$id'");
	}
}
...
class berita {
	...
	function tampil_data(){
		$data=$this->conn->query("SELECT * FROM berita");
		while($d=mysqli_fetch_array($data)){
			$hasil[]=$d;
		}
		return $hasil;
	...
	$this->conn->query("INSERT INTO berita (judul_berita,isi_berita,foto,tanggal,publish) VALUES ('$judul_berita','$isi_berita','$foto','$tanggal','$publish')");
	}

	// tampilkan data dari tabel berita yang akan di edit 
	function edit_berita($id){
	$data=$this->conn->query("SELECT * FROM berita WHERE id_berita='$id'");
	...
	$this->conn->query("UPDATE berita SET judul_berita='$judul_berita',isi_berita='$isi_berita',foto='$foto',publish='$publish' WHERE id_berita='$id'");
	}

	// proses update data berita jika foto tidak diubah
	function update_berita_noupdatephoto($id,$judul_berita,$isi_berita,$publish){
	$this->conn->query("UPDATE berita SET judul_berita='$judul_berita',isi_berita='$isi_berita',publish='$publish' WHERE id_berita='$id'");
	}

	// proses delete data berita
	function delete_berita($id){
	$this->conn->query("DELETE FROM berita WHERE id_berita='$id'");
	}

	function select_foto_to_trash($id){
	$data_gambar = $this->conn->query("SELECT foto FROM berita WHERE id_berita='$id'");
